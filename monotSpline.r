## The function depends on: 'coneproj' (1.11), 'quadprog', 'MASS', and 'orthogonalsplinebasis'
# To install older packages, we recommend: 
...
# monotPSpline 	<- monotSpline(cbind(x,y))
# lines( x = x, y = monotPSpline$constrained.eval, col = "red", lty = 1, lwd = 2)
# lines( x = x, y = monotPSpline$unconstrained.eval, col = "green", lty = 2, lwd = 2)

# 2. Example
# set.seed(1)
# x <- seq(0,1, length.out = 100)
# y <- cumsum(exp(rnorm(100, sd = 5)))/25000
# plot(x,y, type = "l", lwd = 3)
# monotPSpline 	<- monotSpline(cbind(x,y))
# lines( x = x, y = monotPSpline$constrained.eval, col = "red", lty = 1, lwd = 2)
# lines( x = x, y = monotPSpline$unconstrained.eval, col = "green", lty = 2, lwd = 2)
